...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    error.value = err instanceof ApiError ? err.message : '登入失敗,請稍後再試'
  } finally {
...
<template>
  <main class="flex min-h-screen items-center justify-center bg-slate-50 p-4">
    ...
    >
      <h1 class="text-xl font-bold text-slate-800">登入</h1>
      ...
      <p class="text-center text-sm text-slate-500">
        還沒有帳號?
        <RouterLink to="/register" class="text-slate-800 underline">註冊</RouterLink>
      </p>
    </form>
  </main>
</template>
